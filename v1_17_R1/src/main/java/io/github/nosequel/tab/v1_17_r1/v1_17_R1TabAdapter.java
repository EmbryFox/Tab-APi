...
public class v1_17_R1TabAdapter extends TabAdapter {
    ...
    public TabAdapter hideRealPlayers(Player player) {
        EntityPlayer[] entityPlayers = new EntityPlayer[Bukkit.getOnlinePlayers().size()];

        int i = 0;

        for (Player onlinePlayer : Bukkit.getOnlinePlayers()) {
            entityPlayers[i] = ((CraftPlayer) onlinePlayer).getHandle();
            i++;
        }

        PacketPlayOutPlayerInfo packet = new PacketPlayOutPlayerInfo(PacketPlayOutPlayerInfo.EnumPlayerInfoAction.b, entityPlayers);
